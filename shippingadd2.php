...
<form name="sf11" action="upd_shippingadd2.php" method="post">

<?php
    if(isset($_REQUEST['shipuserid']))
    {
	 // echo "user id: ".$_REQUEST['shipuserid'];
	   
	$usids=$_REQUEST['shipuserid'];
   
	$query="select * from shippingadd where uid=$usids";
	$retval=mysqli_query($con,$query);
     while($row=mysqli_fetch_array($retval))
	   {
		
?>  
<input type="hidden" name="usrid" value="<?php echo $usids;?>" >

<br>
<br>
first name
<input type="text" name="fname" value="<?php echo $row['fname'];?>"  required pattern="[A-Z a-z]*" title="please enter first name">
<br>
<br>

last name
<input type="text" name="lname" value="<?php echo $row['lname'];?>" required pattern="[A-Z a-z]*" title="please enter last name">
<br>
<br>

address
<textarea name="add"  required title="please enter address">
...
pincode
<input type="text" name="pincode" value="<?php echo $row['pincode'];?>"  required pattern="\d*" title="please enter pincode">
<br>
<br>

phone no
<input type="text" name="pno" value="<?php echo $row['pno'];?>"  required pattern="[789][0-9]{9}" title="please enter phone number starting with 789">
<br>
<br>
